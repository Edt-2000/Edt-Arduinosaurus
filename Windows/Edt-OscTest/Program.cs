...
namespace Edt_OscTest
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var sender = new UDPSender("192.168.2.45", 12345);
            //var sender = new UDPSender("192.168.0.102", 12345);

            var coll = new List<int>();

            var listener = new UDPListener(12345, (OscPacket data) =>
            {
                var msg = data as OscMessage;
                ...
                coll.Add((int)msg.Arguments[1]);

                Console.WriteLine($"Data {msg.Arguments[0]} - {msg.Arguments[1]} | Avg: {coll.Average()::0.00}, Min: {coll.Min()}, Max {coll.Max()}");
            });

            //var sender = new USB("COM21", 57600);
            ...
            do
            {
                ...
                {
                    sender.Send(new OscMessage("/TD", ++integer, 0));
                    //sender2.Send(new OscMessage("/TD", integer, integer));
                    //sender3.Send(new OscMessage("/TD", integer));
                }
